...
if [ ! -d "agent-engine" ] || [ ! -d "shared" ]; then
    echo -e "${RED}❌ Error: Must run from Alchemist-v1 root directory${NC}"
    echo "Expected to find both 'agent-engine' and 'shared' directories"
    exit 1
fi
...
cat > Dockerfile.agent-engine << EOF
...
cat > .dockerignore.agent-engine << EOF
...
cp .dockerignore.agent-engine .dockerignore
...
- name: 'gcr.io/cloud-builders/docker'
  args: ['build', '-f', 'Dockerfile.agent-engine', '-t', '${IMAGE_NAME}:latest', '.']
- name: 'gcr.io/cloud-builders/docker'
...
# Build environment variables from agent-engine/.env
ENV_VARS=""
if [ -f "agent-engine/.env" ]; then
    echo -e "${BLUE}📋 Loading environment variables from agent-engine/.env...${NC}"
...
rm -f Dockerfile.agent-engine .dockerignore.agent-engine cloudbuild-temp.yaml
