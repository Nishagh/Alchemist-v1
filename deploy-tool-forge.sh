#!/bin/bash
# Deployment script for Alchemist Tool Forge to Google Cloud Run
...
echo -e "${GREEN}🚀 Deploying Alchemist Tool Forge${NC}"
...
if [ -f ".env" ]; then
  ...
  source .env
elif [ -f "tool-forge/.env" ]; then
  echo -e "${BLUE}📋 Loading environment variables from tool-forge/.env file...${NC}"
  source tool-forge/.env
else
...
cat > Dockerfile.tool-forge << EOF
# Multi-stage Docker build for Tool Forge
...
# Install system dependencies (using Cloud Run API instead of gcloud CLI)
...
# Using Cloud Run API instead of gcloud CLI - no CLI configuration needed
...
cat > .dockerignore.tool-forge << EOF
...
# Include what we need for tool-forge
!tool-forge/
!shared/
...
cp .dockerignore.tool-forge .dockerignore
...
- name: 'gcr.io/cloud-builders/docker'
  args: ['build', '-f', 'Dockerfile.tool-forge', '-t', '${IMAGE_NAME}:latest', '.']
- name: 'gcr.io/cloud-builders/docker'
