#!/bin/bash
# Deployment script for Alchemist Billing Service to Google Cloud Run
...
SERVICE_NAME="alchemist-billing-service"
REGION="us-central1"
...
echo -e "${GREEN}🚀 Deploying Alchemist Billing Service${NC}"
...
if [ ! -d "billing-service" ] || [ ! -d "shared" ]; then
    echo -e "${RED}❌ Error: Must run from Alchemist-v1 root directory${NC}"
    echo "Expected to find both 'billing-service' and 'shared' directories"
    exit 1
fi
...
if [ -f ".env" ]; then
  ...
  source .env
elif [ -f "billing-service/.env" ]; then
  echo -e "${BLUE}📋 Loading environment variables from billing-service/.env file...${NC}"
  source billing-service/.env
else
...
cat > Dockerfile.billing-service << EOF
# Multi-stage Docker build for Billing Service
...
cat > .dockerignore.billing-service << EOF
# Exclude all other services except billing-service and shared
admin-dashboard/
agent-bridge/
agent-studio/
...
# Include what we need for billing-service
!billing-service/
!shared/
...
cp .dockerignore.billing-service .dockerignore

# Build the Docker image from root directory
echo -e "${YELLOW}🔨 Building Docker image...${NC}"
docker build --platform linux/amd64 -f Dockerfile.billing-service -t ${IMAGE_NAME}:latest .
...
gcloud run deploy ${SERVICE_NAME} \
    ...
    --concurrency 80 \
    --max-instances 5 \
    --set-env-vars "BILLING_ENVIRONMENT=production" \
...
rm -f Dockerfile.billing-service .dockerignore.billing-service
