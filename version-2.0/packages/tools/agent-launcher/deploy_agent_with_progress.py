...
class AgentDeploymentWithProgress:
    ...
    async def _prepare_build_context(self) -> str:
        ...
        try:
            ...
            if os.path.exists(shared_src):
                ...
                logger.info(f"Copied shared library to build context")
            else:
                raise Exception(f"Shared library not found at {shared_src}")
            
            # Copy agent-launcher directory
            agent_launcher_src = "/app/agent-launcher"
            ...
            if os.path.exists(agent_launcher_src):
                ...
                logger.info(f"Copied agent-launcher to build context")
            else:
                raise Exception(f"Agent-launcher not found at {agent_launcher_src}")
            
            # Copy Dockerfile.agent-template
            dockerfile_src = "/app/Dockerfile.agent-template"
            ...
            if os.path.exists(dockerignore_src):
                ...
                logger.info(f"Copied optimized .dockerignore for agent builds")
            else:
                logger.warning(f".dockerignore.agent-template not found, build may include unnecessary files")
    ...
    async def _build_docker_image(self, build_dir: str):
        ...
        cloudbuild_src = "/app/cloudbuild.agent-template.yaml"
        ...
        if os.path.exists(cloudbuild_src):
            ...
            logger.info("Using predefined cloudbuild.yaml for agent-template")
