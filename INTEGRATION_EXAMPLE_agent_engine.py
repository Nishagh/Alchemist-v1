"""
Example integration of API logging middleware for agent-engine service.

This file shows how to integrate the centralized API logging system into 
the agent-engine service. Similar changes should be made to all other services.

Changes needed in main.py:
1. Import the API logging middleware
2. Add the middleware to the FastAPI app
3. Add shutdown handler for flushing logs
"""
...
# ===== NEW: Import API logging middleware =====
try:
    ...
    logging.warning("API logging middleware not available - install alchemist-shared package")
...
async def lifespan(app: FastAPI):
    ...
    # ===== NEW: Log API logging startup =====
    if API_LOGGING_AVAILABLE:
        logger.info("API logging enabled for agent-engine")
    ...
    # ===== NEW: Shutdown API logging and flush remaining logs =====
    if API_LOGGING_AVAILABLE:
...
# ===== NEW: Add API logging middleware BEFORE other middleware =====
# This should be added first to capture all requests, including those that fail in other middleware
if API_LOGGING_AVAILABLE:
    setup_api_logging_middleware(
        app, 
        service_name="agent-engine",
        service_version="1.0.0",
        exclude_health_checks=True,  # Don't log health checks
        log_request_body=False,      # Don't log request bodies by default for performance
        log_response_body=False,     # Don't log response bodies by default for performance
        max_body_size=1024          # Maximum body size to log if enabled
    )
...
async def root():
    ...
    return {
        ...
        "api_logging_enabled": API_LOGGING_AVAILABLE  # ===== NEW: Indicate if API logging is enabled =====
    }
