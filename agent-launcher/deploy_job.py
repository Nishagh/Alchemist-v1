...
class AgentDeploymentJob:
    ...
    async def process_deployment(self):
        ...
        try:
            ...
            # Execute the deployment using deploy-ai-agent.sh
    ...
    async def _execute_deployment(self, agent_id: str):
        """Execute deployment using deploy-ai-agent.sh script"""
        try:
            # Update status
            await self._update_deployment_status(
                'processing', 
                30, 
                'Deploying agent using deploy-ai-agent.sh...'
            )
            
            # Execute deploy-ai-agent.sh script with agent_id
            deploy_command = ["./deploy-ai-agent.sh", agent_id]
            ...
            if process.returncode != 0:
                ...
                logger.error(f"deploy-ai-agent.sh failed: {error_output}")
            ...
            for line in output.split('\n'):
                if 'Service URL:' in line:
                    # Extract URL from line like "🌐 Service URL: https://..."
                    parts = line.split()
                    for part in parts:
                        if part.startswith('https://'):
                            service_url = part.strip()
                            break
                    if service_url:
                        break
            ...
            # Update agent document with deployment info
